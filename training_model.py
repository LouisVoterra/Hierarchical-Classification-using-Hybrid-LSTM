...
# ACC SUMMARY
print("\n" + "="*55)
print(f"{'FINAL TRAINING SUMMARY':^55}")
print("="*55)
print(f"{'Output Level':<15} | {'Train Acc':<15} | {'Val Acc':<15}")
print("-" * 55)

for i in range(1, 4):
    key_acc = f'output_l{i}_accuracy'
    key_val = f'val_output_l{i}_accuracy'
    
    acc = history.history[key_acc][-1] * 100
    val = history.history[key_val][-1] * 100
    
    print(f"{f'Level {i}':<15} | {acc:.2f}%          | {val:.2f}%")
